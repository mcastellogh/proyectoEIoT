...
\begin{itemize}
\item Se producirán las placas de circuito impreso para los nodos sensores de temperatura y se integrarán en un gabinete para poder ser colocados en el equipamiento a monitorear.

\item Se instalará y configurará en un servidor con sistema operativo GNU/Linux distribución Debian, el panel de control (dashboard) seleccionada, que proveerá la visualización de los datos capturados por los sensores.
%Thingsboard versión Community Edition que es la versión libre de licenciamientos pero que contiene todas las funcionalidades necesarias para este proyecto.

\item Se configurarán reglas en la dashboard para comparar las temperaturas de los dispositivos con sus rangos para así originar las alarmas y enviar las notificaciones.

\item Se instalará una base de datos en el servidor central donde se almacenarán las temperaturas, y las configuraciones del sistema.

\item Se configurarán en la dashboard paneles para que proporcionen información de temperatura y estado para cada dispositivo conectado al sistema.

\item Por medio de la red Telegram se enviarán las notificaciones de alarmas. Se configurarán canales de distribución de mensajes para cada efector/área.
\end{itemize}

El presente proyecto no incluye.
\begin{itemize}
\item La provisión de la infraestructura de transporte de datos, esto es, los equipos para puntos de acceso a Internet y las conexiones a Internet para cada área donde estarán emplazados los sensores.
\item Las certificaciones emitidas por las autoridades competentes.
\item Los nodos sensores desarrollados no serán aptos para su instalación en equipos ultrafreezer (-70º).
\item Los nodos sensores no almacenarán los valores de temperaturas en memorias externas tipo SD ni en la memoria interna del microcontrolador.
\item Los nodos sensores no usarán baterías para su funcionamiento.
\end{itemize}
...
Dado que en la Municipalidad de Rosario los trámites necesarios para llevar a cabo una gestión de compras en el exterior son complejos y harían que el proyecto sufra demoras muy importantes, se preve realizar una reunión con las autoridades tanto políticas como administrativas para lograr una agilización en esta gestión de compra. En dicha reunión se pondrá de manifiesto la importancia del proyecto en el contexto que se avecina referido al almacenamiento seguro de especialidades medicinales.
